...
export function BlogPage() {
  ...
  useEffect(() => {
    supabase
      .from('blog_articles')
      .select('id, title, slug, excerpt, category, cover_image, cover_image_alt, published_at, views')
    ...
    <div className="min-h-screen px-4 py-8" style={{ background: '#0a0f2e' }}>
      <div className="max-w-6xl mx-auto">
        ...
        {!loading && filtered.length === 0 && (
          <p className="text-center" style={{ color: '#a0aec0' }}>Aucun article publié pour le moment.</p>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((article) => (
            <Link
              ...
              style={{ background: '#0d1545', border: '1px solid rgba(245,200,66,0.15)' }}
            >
              <div
              ...
              <div className="p-5 flex flex-col gap-2 flex-1">
                ...
                <p className="text-sm flex-1" style={{ color: '#a0aec0' }}>{article.excerpt}</p>
